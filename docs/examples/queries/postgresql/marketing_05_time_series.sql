...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN es.delivered THEN es.send_id END) as delivered,
...
LEFT JOIN marketing_campaigns__email_opens eo ON es.send_id = eo.send_id AND es.send_date = eo.open_date
LEFT JOIN marketing_campaigns__email_clicks ec ON es.send_id = ec.send_id AND es.send_date = ec.click_date
